...
public class AggFuncWithLimitTest {
    ...
    void testAggFuncWithLimit() {
        ...
        AbstractRawDataAggFunc.RawDataAccumulator<Integer> acc1 = aggFunc.createAccumulator();
        aggFunc.add(acc1, 1, 1L);
        aggFunc.add(acc1, 2, 2L);
        assertThat(aggFunc.getResult(acc1)).isEqualTo(1);

        AbstractRawDataAggFunc.RawDataAccumulator<Integer> acc2 = aggFunc.createAccumulator();
        aggFunc.add(acc2, 3, 3L);
        aggFunc.add(acc2, 4, 4L);
        assertThat(aggFunc.getResult(acc2)).isEqualTo(3);

        AbstractRawDataAggFunc.RawDataAccumulator<Integer> acc = aggFunc.createAccumulator();
