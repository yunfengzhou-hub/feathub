...
public abstract class SumAggFunc<IN_T>
    ...
    @Override
    public SumAggFunc.SumAccumulator<IN_T> merge(
            SumAggFunc.SumAccumulator<IN_T> target, SumAggFunc.SumAccumulator<IN_T> source) {
        return add(target, source.value, -1L);
    }

    @Override
    public SumAggFunc.SumAccumulator<IN_T> retractAccumulator(
            SumAggFunc.SumAccumulator<IN_T> target, SumAggFunc.SumAccumulator<IN_T> source) {
        return retract(target, source.value);
    }
    ...
    public static class IntSumAggFunc extends SumAggFunc<Integer> {
        ...
        @Override
        public SumAccumulator<Integer> add(
                SumAccumulator<Integer> accumulator, Integer value, long timestamp) {
            accumulator.value += value;
            return accumulator;
        }

        @Override
        public SumAccumulator<Integer> retract(SumAccumulator<Integer> accumulator, Integer value) {
            accumulator.value -= value;
            return accumulator;
        }
    ...
    public static class LongSumAggFunc extends SumAggFunc<Long> {
        @Override
        public SumAccumulator<Long> add(
                SumAccumulator<Long> accumulator, Long value, long timestamp) {
            accumulator.value += value;
            return accumulator;
        }

        @Override
        public SumAccumulator<Long> retract(SumAccumulator<Long> accumulator, Long value) {
            accumulator.value -= value;
            return accumulator;
        }
    ...
    public static class FloatSumAggFunc extends SumAggFunc<Float> {
        @Override
        public SumAccumulator<Float> add(
                SumAccumulator<Float> accumulator, Float value, long timestamp) {
            accumulator.value += value;
            return accumulator;
        }

        @Override
        public SumAccumulator<Float> retract(SumAccumulator<Float> accumulator, Float value) {
            accumulator.value -= value;
            return accumulator;
        }
    ...
    public static class DoubleSumAggFunc extends SumAggFunc<Double> {
        @Override
        public SumAccumulator<Double> add(
                SumAccumulator<Double> accumulator, Double value, long timestamp) {
            accumulator.value += value;
            return accumulator;
        }

        @Override
        public SumAccumulator<Double> retract(SumAccumulator<Double> accumulator, Double value) {
            accumulator.value -= value;
            return accumulator;
        }
