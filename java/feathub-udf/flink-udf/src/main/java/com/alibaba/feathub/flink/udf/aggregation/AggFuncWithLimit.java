...
public class AggFuncWithLimit<IN_T, PRE_OUT_T, OUT_T, PRE_ACC_T, ACC_T>
        implements AggFunc<RawDataAccumulator<IN_T>, OUT_T, RawDataAccumulator<IN_T>> {
    ...
    @Override
    public void add(
            RawDataAccumulator<IN_T> target, RawDataAccumulator<IN_T> source, long timestamp) {
        for (Tuple2<IN_T, Long> rawData : source.rawDataList) {
    ...
    @Override
    public void retract(RawDataAccumulator<IN_T> target, RawDataAccumulator<IN_T> source) {
