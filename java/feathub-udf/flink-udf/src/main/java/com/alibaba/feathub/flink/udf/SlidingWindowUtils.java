...
public class SlidingWindowUtils {
     ...
     */
    public static DataStream<Row> applySlidingWindowPreprocessAggregateFunction(
        ...
        accumulatorFieldTypes.add(Types.LONG);
        for (AggregationFieldsDescriptor.AggregationFieldDescriptor aggDescriptor :
                aggDescriptors.getAggFieldDescriptors()) {
            fieldNames.add(aggDescriptor.outFieldName);
            resultFieldTypes.add(aggDescriptor.preAggFunc.getResultTypeInformation());
            accumulatorFieldTypes.add(aggDescriptor.preAggFunc.getAccumulatorTypeInformation());
        }
        ...
        return stream.union(lateDataStream);
    }
    ...
    private static class SlidingWindowPreprocessAggregateFunction
        ...
        private final Map<String, PreAggFunc> preAggFuncMap;
        ...
        public SlidingWindowPreprocessAggregateFunction(
            ...
            this.preAggFuncMap = new HashMap<>();
            for (AggregationFieldsDescriptor.AggregationFieldDescriptor descriptor :
                    aggDescriptors.getAggFieldDescriptors()) {
                this.preAggFuncMap.put(descriptor.outFieldName, descriptor.preAggFunc);
            }
            this.size = size;
            this.offset = offset;
        ...
        public Row createAccumulator() {
            ...
            for (Map.Entry<String, PreAggFunc> aggFieldName : preAggFuncMap.entrySet()) {
                acc.setField(aggFieldName.getKey(), aggFieldName.getValue().createAccumulator());
            }
            return acc;
        }
        ...
        public Row add(Row row, Row acc) {
            ...
            for (Map.Entry<String, PreAggFunc> entry : preAggFuncMap.entrySet()) {
                Object value = row.getFieldAs(entry.getKey());
                entry.getValue().add(acc.getField(entry.getKey()), value, timestamp);
            }

            if (acc.getField(rowTimeFieldName) == null) {
                acc.setField(rowTimeFieldName, getWindowTime(timestamp, size, offset));
        ...
        public Row getResult(Row acc) {
            ...
            for (Map.Entry<String, PreAggFunc> entry : preAggFuncMap.entrySet()) {
                result.setField(
                        entry.getKey(), entry.getValue().getResult(acc.getField(entry.getKey())));
            }

            result.setField(
                    rowTimeFieldName, Instant.ofEpochMilli(acc.getFieldAs(rowTimeFieldName)));

            for (String key : keyFields) {
                result.setField(key, acc.getField(key));
            }
        ...
        public Row merge(Row acc1, Row acc2) {
            for (Map.Entry<String, PreAggFunc> entry : preAggFuncMap.entrySet()) {
                entry.getValue()
                        .merge(acc1.getField(entry.getKey()), acc2.getField(entry.getKey()));
            }
    ...
    private static class LateDataToPreAggResultMapFunction implements MapFunction<Row, Row> {
        ...
        private LateDataToPreAggResultMapFunction(
            ...
            this.preAggFuncMap = new HashMap<>();
            for (AggregationFieldsDescriptor.AggregationFieldDescriptor descriptor: aggDescriptors.getAggFieldDescriptors()) {
                this.preAggFuncMap.put(descriptor.outFieldName, descriptor.preAggFunc);
            }
        ...
        public Row map(Row row) throws Exception {
            ...
            for (Map.Entry<String, PreAggFunc> entry: preAggFuncMap.entrySet()) {
                ...
                PreAggFunc preAggFunc = entry.getValue();
                String value = row.getFieldAs(fieldName);
                Object acc = preAggFunc.createAccumulator();
                preAggFunc.add(acc, value, timestamp);
                result.setField(fieldName, preAggFunc.getResult(acc));
            }

            result.setField(
                    rowTimeFieldName,
                    Instant.ofEpochMilli(
                            getWindowTime(timestamp, stepSize, offset)));

            result.setField("isLateData", true);

            for (int i = 0; i < keyFieldNames.size(); i++) {
                result.setField(keyFieldNames.get(i), row.getField(i));
            }
            return result;
        }
    }
