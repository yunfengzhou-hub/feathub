...
class MetricStore(ABC):
    ...
    def instantiate(props: Dict) -> Optional["MetricStore"]:
        ...
        registry_config = MetricStoreConfig(props)
        registry_type = MetricStoreType(registry_config.get(METRIC_STORE_TYPE_CONFIG))

        if registry_type is None:
            return None

        elif registry_type == MetricStoreType.PROMETHEUS:
    ...
    def namespace(self) -> str:
        return self._config.get(METRIC_STORE_NAMESPACE_CONFIG)

    @property
    def report_interval(self) -> timedelta:
        return self._config.get(METRIC_STORE_REPORT_INTERVAL_CONFIG)
