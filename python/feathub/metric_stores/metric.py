...
from typing import Optional, Dict, Union, OrderedDict, Sequence, Callable
...
class Metric(ABC):
    ...
    def get_tags(self) -> OrderedDict:
        ...
        return collections.OrderedDict([
            ("window_time", str(int(self.window_size / timedelta(seconds=1))))
        ])

    @abstractmethod
    def get_transformations(self) \
            -> Sequence[Callable[[str], Union[str, Transformation]]]:
        """
        Gets the functions to generate transformations used to aggregate and compute the metric value.
        The function has the following parameter:

        - metric_name: Name of A variable containing the intermediate metric value generated from
                       previously defined transformation. For the first transformation, its
                       value is the host feature name.
        """
        pass
    ...
    def get_transformations(self) \
        ...
        return [
            ...
            SlidingWindowTransform(
                ...
                filter_expr=f"{metric_name} {self.filter_expr}",
            )
        ]
...
class Ratio(Metric):
    ...
    def get_transformations(self) \
        ...
        return [
            lambda metric_name:
            SlidingWindowTransform(
                expr=f"CASE WHEN {metric_name} {self.filter_expr} " f"THEN 1 ELSE 0 END",
                agg_func="AVG",
                window_size=self.window_size,
                step_size=self.window_size,
            ),
            lambda metric_name:
            f"CASE WHEN {metric_name} IS NULL THEN 0 ELSE "
            f"{metric_name} END"
        ]
