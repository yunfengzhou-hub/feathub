...
class UtilsTest(unittest.TestCase):
    ...
    def test_read_write_protobuf(self):
        ...
        serialized_data = [[None for _ in row] for row in original_data]
        ...
        for i in range(len(original_data)):
            for j in range(len(schema.field_names)):
                deserialized_data[i][j] = utils.deserialize_protobuf_object(
                    serialized_data[i][j], schema.field_types[j]
                )
