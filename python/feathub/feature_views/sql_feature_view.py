...
from typing import Dict, Union, Sequence, Optional, List, cast
...
class SqlFeatureView(FeatureView):
    """
    Acquires features by evaluating the given SQL statement using a specific Processor.

    Apart from the following limitations, The SQL statement may use any sql
    ...
    - The SQL statement should end with creating a view with the same name as the
    FeatureView, which contains the features to be presented through this FeatureView.
    ...
    def __init__(
        ...
        :param sources: The sources form which the SQL statement derives features.
        :param sql_statement: The SQL statement to be evaluated with an internal
                              processor.
        ...
        :param is_bounded: Whether the SqlFeatureView is bounded. By default it is
                           bounded iff all of its sources are bounded.
        """
        super().__init__(
            name=name,
            keys=keys,
    ...
    def build(
        ...
        for source in self.sources:
            if isinstance(source, str):
                sources.append(registry.get_features(name=source))
            else:
                sources.append(registry.build_features(features_list=[source])[0])
    ...
    def is_unresolved(self) -> bool:
        return any(
            ...
            or (isinstance(source, FeatureView) and source.is_unresolved())
            for source in self.sources
        )
    ...
    def get_bounded_view(self) -> TableDescriptor:
        if self.is_bounded():
            return self
        raise FeathubException(f"Cannot get bounded view of table {self.name}.")

    def is_bounded(self) -> bool:
        if self._is_bounded is not None:
            return self._is_bounded
        if self.is_unresolved():
            raise RuntimeError("This feature view is unresolved.")
        return all(
            cast(TableDescriptor, source).is_bounded() for source in self.sources
        )
    ...
    def to_json(self) -> Dict:
        return {
            ...
            "name": self.name,
            "sources": [
                source if isinstance(source, str) else source.to_json()
                for source in self.sources
            ],
            "sql_statement": self.sql_statement,
