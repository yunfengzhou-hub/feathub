...
class GetTableTestBase(ProcessorTestBase):
    ...
    def test_get_table_with_single_key(self):
        source = self._create_file_source(
            self.input_data.copy(), schema=self.schema, keys=["name"]
        )
        keys = pd.DataFrame(
            [
                ["Alex"],
    ...
    def test_get_table_with_multiple_keys(self):
        ...
        source = self._create_file_source(df, schema=self.schema, keys=["name"])
        keys = pd.DataFrame(
            [
                ["Alex", 100],
                ["Alex", 200],
    ...
    def test_get_table_missing_timestamp(self):
        ...
        source = self._create_file_source(
            df, schema=self.schema, keys=["name"], timestamp_field=None
        )
