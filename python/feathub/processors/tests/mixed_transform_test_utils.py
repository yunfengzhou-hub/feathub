...
class MixedTransformTestBase(ProcessorTestBase):
    """
    ...
    def test_python_udf_transform_on_over_window_transform(self):
        ...
        result_df = (
            self.processor.get_table(features=features)
            .to_pandas()
    ...
    def test_join_sliding_feature(self):
        ...
        for props, expected_result_df in expected_results:
            ...
            print(built_joined_feature)

            result_df = (
                self.processor.get_table(features=built_joined_feature)
                .to_pandas()
    ...
    def test_sliding_window_with_python_udf(self):
        ...
        source = self._create_file_source(df, schema=self.schema)
    ...
    def test_over_window_on_join_field(self):
        ...
        source = self._create_file_source(df_1, schema=self.schema)
        ...
        result_df = (
            self.processor.get_table(features=built_feature_view_2)
            .to_pandas()
            .sort_values(by=["name", "time"])
            .reset_index(drop=True)
        )
