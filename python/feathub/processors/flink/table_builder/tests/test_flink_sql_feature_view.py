...
class FlinkSqlFeatureViewITTest(ABC, FeathubITTestBase):
    def setUp(self) -> None:
        ...
        source = self.create_file_source(self.input_data.copy(), name="__TEST_SOURCE__")
    ...
    def test_sql_feature_view(self):
        features = SqlFeatureView(
            name="test_view_name",
            sql_statement="""
                SELECT name, cost FROM __TEST_SOURCE__;
            """,
            schema=(
                Schema.new_builder()
    ...
    def test_usage_with_udf(self):
        features = SqlFeatureView(
            name="test_view_name",
            sql_statement="""
                SELECT *, UPPER(name) as upper_name FROM __TEST_SOURCE__;
            """,
            schema=(
                Schema.new_builder()
                .column("name", types.String)
    ...
    def test_usage_followed_by_sliding_feature_view(self):
        features: FeatureView = SqlFeatureView(
            name="test_view_name",
            sql_statement="""
                SELECT name, cost, `time`, __event_time_attribute__
                FROM __TEST_SOURCE__;
            """,
            schema=(
                Schema.new_builder()
                .column("name", types.String)
    ...
    def test_usage_followed_by_sql_feature_view(self):
        features = SqlFeatureView(
            name="test_view_name",
            sql_statement="""
                 SELECT name, cost FROM __TEST_SOURCE__;
        ...
        features = SqlFeatureView(
            ...
            sql_statement="""
                SELECT name FROM test_view_name;
            """,
            schema=(Schema.new_builder().column("name", types.String).build()),
        )
