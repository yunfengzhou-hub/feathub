...
class SlidingWindowDescriptor:
    ...
    def to_java_descriptor(self) -> Any:
        ...
        return gateway.jvm.com.alibaba.feathub.flink.udf.SlidingWindowDescriptor(
            gateway.jvm.java.time.Duration.ofSeconds(
                int(self.step_size.total_seconds())
            ),
            self.limit,
            self.group_by_keys,
            self.filter_expr,
...
def evaluate_sliding_window_transform(
    ...
    j_stream = SlidingWindowUtils.applySlidingWindowPreprocessAggregateFunction(
    ...
    j_table = SlidingWindowUtils.applySlidingWindowKeyedProcessFunction(  # noqa
        t_env._j_tenv,
        j_stream,
        data_type_map,
        group_by_keys,
