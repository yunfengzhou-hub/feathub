...
def to_sql_type(input_type: Union[DType, DataType]) -> str:
    ...
    if isinstance(flink_type, VarCharType):
        ...
        return f"ARRAY<{to_sql_type(flink_type.element_type)}>"
    elif isinstance(flink_type, NativeFlinkMapType):
        return (
            f"MAP<{to_sql_type(flink_type.key_type)}, "
            f"{to_sql_type(flink_type.value_type)}>"
        )
